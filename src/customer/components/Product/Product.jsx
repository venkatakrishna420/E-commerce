...
export default function Product() {
  ...
  const handleFilter = (value, sectionId) => {
    ...
    const serchParamms = new URLSearchParams(location.search)

    let filterValue = serchParamms.getAll(sectionId)
    if (filterValue.length > 0 && filterValue[0].split(",").includes(value)) {
      filterValue = filterValue[0].split(",").filter((item) => item !== value);

      if (filterValue.length === 0) {
        serchParamms.delete(sectionId)
      }
    }
    else {
    ...
    if (filterValue.length > 0) {
      serchParamms.set(sectionId, filterValue.join(","));

    }
    const query = serchParamms.toString();
